...
function Fano=FanoFactorOfSpikeCounts(s,N,T1,T2,winsize,Inds,BlockTrial,TrialTime,...
...
%     end
    ...
    if(Control)
        ...
        % Compute variance and mean of spike count to each neuron.
        mean_counts = mean(Counts,1);
        var_counts = var(Counts,1);
        % Compute Fano factor of each neuron
